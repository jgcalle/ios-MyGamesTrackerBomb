//
//  GamesByTrackerMustHavesCDTVC.m
//  MyGamesTracker
//
...
#import "GamesByTrackerMustHavesCDTVC.h"
#import "GameDatabaseAvailability.h"
#import "Tracker+MyGamesTracker.h"
#import "Item+MyGamesTracker.h"
#import "Game+GiantBomb.h"

@interface GamesByTrackerMustHavesCDTVC ()

@property (strong, nonatomic) Tracker *trackerMustHaves;
...
@implementation GamesByTrackerMustHavesCDTVC
...
- (void)setTitleFetchedObjects:(int)count
{
    if (count > 0) {
        ...
        self.navigationItem.title = [NSString stringWithFormat:@"%@ %@",NSLocalizedString(@"STRING_TITLE_TABLE_MUST", nil),title];
    } else {
        ...
        self.navigationItem.title = NSLocalizedString(@"STRING_TITLE_TABLE_MUST", nil);
    }
}
...
- (void)setupFetchedResultsController
{
    ...
    if (self.managedObjectContext) {
        ...
        self.trackerMustHaves = [Tracker getMyTrackerMustHavesinManagedObjectContext:self.managedObjectContext];
        
        if (self.trackerMustHaves) {
            ...
            request.predicate = [NSPredicate predicateWithFormat:@"items.tracker.name CONTAINS[cd] %@", self.trackerMustHaves.name];
